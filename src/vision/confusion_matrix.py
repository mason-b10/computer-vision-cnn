...
def generate_confusion_data(
    ...
    for key in idx_to_label:
        class_labels[key] = idx_to_label[key]
...
def generate_confusion_matrix(
    ...
    if normalize:
        norm = np.reshape(np.sum(confusion_matrix, axis=1), (confusion_matrix.shape[0], 1))
        confusion_matrix = confusion_matrix / norm
...
def generate_accuracy_table(
    ...
    correct = np.where(preds == targets, 1, 0)
    accuracy_table = np.sum(correct, axis=0) / preds.shape[0]
