...
def compute_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    ...
    acc = torch.count_nonzero(output == labels)
    batch_accuracy = acc / logits.shape[0]
...
def compute_loss(
    ...
    if "Multilabel" in model.__class__.__name__:
        target_labels = target_labels.type(torch.float)
...
def compute_multilabel_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    ...
    rows, cols = predictions.size()
    totalPred = rows * cols
    batch_accuracy = torch.count_nonzero(acc) / totalPred
